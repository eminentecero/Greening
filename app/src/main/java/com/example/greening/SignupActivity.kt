...
class SignupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        button_idCheck.setOnClickListener {
            ...
            sqlDB = openOrCreateDatabase("sqlDataBase", MODE_PRIVATE,null)
            var dbCursor : Cursor = sqlDB.rawQuery("SELECT * FROM ID", null)

            val idList = Array<String>()
            val passwordList = Array<String>()

            if(dbCursor != null){
                if(dbCursor.moveToFirst()){
                    do{
                        idList.add(dbCursor.getString(dbCursor.getColumnIndex("idList")))
                        passwordList.add(dbCursor.getString(dbCursor.getColumnIndex("passwordList")))
